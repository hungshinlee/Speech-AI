...
def main():
    ...
    written.append(write(os.path.join(ROOT, "slides.qmd"), "\n".join([
        ...
        "",
        "Speaker notes are in Chinese, and they are not private: press `S`, or add "
        "`?showNotes=true` to the URL, and anyone can read them.",
        "",
        "::: {.callout-tip}",
        "## Keyboard shortcuts",
        "`S` presenter view (with the Chinese speaker notes) · `B` chalkboard layer "
        "for writing on a slide · `ESC` slide overview · `F` full screen · "
        "add `?print-pdf` to the URL to print",
        ":::",
        "",
    ] + rows + [
        "",
