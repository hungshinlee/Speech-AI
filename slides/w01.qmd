...
## ...and the one almost nobody writes down {.smaller}
...
## Humans answer faster than they can plan {.smaller}
...
## Misconception 3 — "just make the VAD faster" {.smaller}
...
## Two rules before you fill in a single cell {.smaller}
...
**Computational delay** — this is the one you can buy your way out of:

$$D_{\text{comp}} \;=\; \mathrm{RTF}\cdot C\Delta \;+\; T_{\text{queue}}$$
:::

::: {.claim}
$D_{\text{e2e}} = D_{\text{alg}} + D_{\text{comp}} + D_{\text{net}} + D_{\text{buf}}$ — every term counted **after the user stops speaking**. A faster GPU moves exactly one of them.
:::

## The arithmetic that eats the whole budget {.smaller}
...
Why not just shrink $C$? Chunks arrive at $1/(C\Delta)$ per second, and each costs $O\!\big(C(C+H)d\big)$ — so the per-second cost is $O\!\big((C+H)d/\Delta\big)$: **smaller chunks are not cheaper per second, they are more often and less efficient.**

## Misconception 4 — "latency is an engineering problem, not a research problem" {.smaller}
...
## The other budget: how long it takes to stop {.smaller}

Everything in the last section measured one direction: *user stops → system starts*. Interruption runs the other way.

| *user starts → system stops* | |
|:--|:--|
| Detection | noticing the user has started at all |
| Decision | backchannel to talk through, or barge-in to yield to? |
| **Committed buffer** | audio already handed to the sound card. It will be heard. It cannot be recalled. |

::: {.claim .warn}
The third one is the trap. A larger playout buffer protects you from glitches **and** makes you slower to shut up. One parameter, two faces — and only one of them shows up in the usual latency number.
:::

Recording 1 was **2.90 s** of exactly this. `analyze_duplex_audio.py` reports three numbers — response latency, overlap, stop latency — because a system that answers fast and stops slowly is still a bad listener.

## Misconception 5 — "the user should always win an interruption" {.smaller}
