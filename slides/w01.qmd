...
...and the one almost nobody writes down: **the clock.** When each word landed. How long that pause in the middle was. How long ago the user stopped.
...
- **$\ell$ is not a knob you set once.** It is the *sum of every layer's right context*. Fifteen layers each looking two frames ahead is thirty frames — 300 ms, gone before a single multiply happens.
