...
::: {.footer}
Audio: Ohashi, Zeghidour, Défossez & Kharitonov, *Multi-Faceted Interactivity Alignment in Full-Duplex Speech Models*, EMNLP 2026 (arXiv:2606.11167). Samples from `kyutai/interactivity-alignment-samples`, CC-BY-4.0. Stereo: left = user, right = model.
:::
...
::: {.footer}
Energy-based VAD on each channel (25 ms frames, 10 ms hop) — an estimate, not a reference VAD. Analysis script: `scripts/analyze_duplex_audio.py`.
:::
...
Not "know about" — **read the papers critically, measure the systems, and defend design choices.**

| Part | Weeks | What it gives you |
|---|---|---|
| **I — Foundations** | W2–W6 | signals and front ends; alignment (HMM → CTC); Transformers under a streaming constraint; self-supervised representations; neural codecs and discrete tokens |
| **II — Modules** | W7–W10 | ASR; TTS generative paradigms; controllability and evaluation; the front end that full duplex cannot avoid |
| **III — Dialogue systems** | W11–W14 | audio-native LMs; turn-taking; full-duplex architectures and data; evaluation, alignment, deployment |
...
Everything in Part III is built on three ideas introduced in W6:

- **frame rate** — how many tokens per second of speech, and therefore how many decoder steps per second
- **token budget** — how much of the language model's context a 30-minute conversation costs
- **semantic / acoustic disentanglement** — whether the model must learn meaning and timbre in the same representation

::: {.claim}
Speech only becomes something a language model can consume once you have decided how to turn it into tokens. Every downstream latency and quality ceiling follows from that decision.
:::
...
The LLM receives a string. It cannot know that the user was hesitant, amused, shouting, or speaking very fast — nor that they had not finished.

::: {.claim .warn}
The text bottleneck is not a quality problem you can fix with a better ASR. It is an **information-theoretic** one: the channel is too narrow for the signal.
:::
...
::: {.claim}
Once the model must emit *something* at every frame — including "say nothing" — turn-taking stops being a scheduling problem and becomes ordinary next-token prediction.
:::
...
## Which failure can you afford? {.smaller}

| | Cascade | End-to-end S2S | Native full duplex |
|---|---|---|---|
| Paralinguistics | lost at the ASR output | preserved | preserved |
| Turn-taking | external VAD / endpointer | external, usually | modelled per frame |
| Semantic ability | full text LLM | degrades; mitigated by interleaving | same, plus timing to learn |
| Controllability, tools, audit | strongest | weaker | weakest today |
| Training cost | lowest | high | highest; needs two-track dialogue data |
| Debuggability | per module | end to end only | end to end only |

::: {.claim}
There is no ranking here. Pick the architecture whose failure mode your application can survive.
:::
...
::: {.footer}
Stivers, Enfield, Brown, … Levinson, *Universals and cultural variation in turn-taking in conversation*, PNAS 106(26): 10587–10592, 2009. Open access.
:::
...
::: {.footer}
Audio: Ohashi et al., EMNLP 2026 (arXiv:2606.11167); `kyutai/interactivity-alignment-samples`, CC-BY-4.0.
:::
...
::: {.metrics}
::: {.metric}
[0.88 s → 4.20 s]{.num}[simultaneous speech, base → post-trained]{.lab}
:::
::: {.metric}
[3 → 11]{.num}[model utterances over the same 42 s]{.lab}
:::
:::
