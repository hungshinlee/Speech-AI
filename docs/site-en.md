...
Hand this diagram out in week 1, then fill in each module's contribution as the course goes. The median response time at a turn transition in human conversation is on the order of 200 ms (search *turn-taking gap distribution, Levinson & Torreira, universals in turn-taking timing*); that is where the engineering target for "natural" comes from.
